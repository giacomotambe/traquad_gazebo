...
set(_exported_dependencies "builtin_interfaces;controller_interface;hardware_interface;pluginlib;rclcpp_lifecycle;rclcpp;sensor_msgs;pi3hat_moteus_int_msgs;std_srvs;custom_interfaces")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to wheels_vel_cnt_DEFINITIONS, wheels_vel_cnt_INCLUDE_DIRS,
# wheels_vel_cnt_LIBRARIES, and wheels_vel_cnt_LINK_FLAGS.
# Additionally collect the direct dependency names in
# wheels_vel_cnt_DEPENDENCIES as well as the recursive dependency names
# in wheels_vel_cnt_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(wheels_vel_cnt_DEPENDENCIES ${_exported_dependencies})
  set(wheels_vel_cnt_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(wheels_vel_cnt_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(wheels_vel_cnt_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(wheels_vel_cnt_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(wheels_vel_cnt_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(wheels_vel_cnt_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND wheels_vel_cnt_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
