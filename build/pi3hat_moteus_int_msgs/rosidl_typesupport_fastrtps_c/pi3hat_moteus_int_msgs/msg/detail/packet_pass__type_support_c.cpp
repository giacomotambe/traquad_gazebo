  ...
  void * untyped_ros_message)
{
  ...
  // Field name: pack_loss
  {
    ...
    if (!rosidl_runtime_c__double__Sequence__init(&ros_message->pack_loss, size)) {
      fprintf(stderr, "failed to create array for field 'pack_loss'");
      return false;
    }
    auto array_ptr = ros_message->pack_loss.data;
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = pi3hat_moteus_int_msgs__msg__PacketPass;
    is_plain =
      (
...
static size_t _PacketPass__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_pi3hat_moteus_int_msgs__msg__PacketPass(
